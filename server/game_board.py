...
class Object():
    ...
    """
    def __init__(self,imageDir,name,imageFile,type,x,y,dir,eyes_open=False,color ="white",width=0,heigh=0):
        self.name = name
        self.x = x
        self.y = y
...
#------------------------------------------------------------
class Player(Object):
    def __init__(self,imageDir,name,image_name,x,y,dir,view_type,color="white",width=0,heigh=0):
        super().__init__(imageDir,name, image_name,"bitmap",x,y,dir,color,width,heigh)
...
class GameBoard(tk.Frame):
    ...
    def __init__(self, parent, columns=16,rows=16, size=64, color1="white", color2="grey"):
        ...
        self.color2 = color1
        self.objects = []
        self.places_visited = [] # Places where agents or agent already were.

        canvas_width = columns * size
        canvas_height = rows * size
        #compreension of the matrix
        self.rectangles=[[0 for x in range(rows)] for x in range(columns)]
        self.weightpattern = [[0 for x in range(rows)] for x in range(columns)]
...
#        self.qB_window = self.canvas.create_window(10, 10, window=self.quitButton)
    ...
    #------------------------------------------------
    # SET_PLACES_VISITED and GET_PLACES_VISITED (2020)
    # Places visited by agents
    #------------------------------------------------
    def set_placesVisited(self,place):
        self.places_visited.append(place)
    def get_placesVisited(self):
        return self.places_visited

    def is_inPlaceVisited(self, place):
        if place in self.places_visited:
            return True
        else:
            return False
    #------------------------------------------------
    ...
    # ------------------------------------------------
    # PRINT_POSITION: (place an object color where hte object is.
    # ------------------------------------------------
    def print_position(self, object, x, y):
        """Set the step of the object, giving the color yellow to the patch"""
        #test
        print("Color of the object is: ", object.get_color())
        self.canvas.itemconfig(self.rectangles[object.get_x()][object.get_y()], fill=object.get_color())
...
#        y = self.change_y(y)
    ...
    # Move To
    #------------------------------------------------
    def move_to(self,object,position):
        #self.place(object,position[0],position[1])
        #Test
...
#        self.change_position(object, x, y)
    ...
    def move_south(self,object,movement):
        if movement =="forward":
            ...
            x = object.get_x()
            y = object.get_y() - 1
#       self.change_position(object, x, y)print_step
        x = self.change_x(x)
        y = self.change_y(y)
        return (x,y)
...
#        self.change_position(object, x, y)
    ...
    #------------------------------------------------
    def getgoalsposition(self, object):
        posgoals =[]
        for ag in self.objects:
            if isinstance(ag,Goal):
                posgoals.append((ag.get_x(), ag.get_y()))
        return posgoals

    #-------------------------------------------------
    # GETAGENTPOSITION
    #-------------------------------------------------
    def getagentposition(self,object):
        return (object.get_x(),object.get_y())
    ...
    def view_obstacles(self, object):
        obstacles = [[0 for x in range(self.rows)] for x in range(self.columns)]
        for ag in self.objects:
